...
{
    public partial class frmReporteCTS : FormGradient
    {
        public frmReporteCTS()
        {
            InitializeComponent();
        }
        ...
        DateTime FechaInicial, FechaFinal; private void MostrarDatosFiltrados()
        {
            if (Cargado)
            {
                ...
                TDatos = CapaLogica.CTSReporte(txtbusEmpresa.TextValido(), txtbusEmpleado.TextValido(), FechaInicial, FechaFinal);
                dtgconten.DataSource = TDatos;
                lblRegistros.Text = $"Total Registros: {dtgconten.RowCount}";
            }
        }
        private void frmReporteCTS_Load(object sender, EventArgs e)
        {
        ...
        private void backgroundWorker1_DoWork(object sender, DoWorkEventArgs e)
        {
            if (dtgconten.RowCount > 0)
            {
                ...
                //
                _NombreHoja = $"Resumen de CTS {FechaInicial.ToString("dd-MM-yyyy")} Al {FechaFinal.ToString("dd-MM-yyyy")}".ToUpper();
                _Cabecera = "Resumen de las CTS";
                _NColumna = "F";
