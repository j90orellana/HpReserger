...
namespace HPReserger
{
    public partial class frmEmpresaEps : Form
    {
        ...
        {
            dtgconten.DataSource = CCargos.getCargoTipoContratacion("ID_Eps", "Empresa_Eps", "TBL_Empresa_Eps");
            dtgconten.Focus();
        }
        ...
        private void btnaceptar_Click(object sender, EventArgs e)
        {
            if (estado == 1)
            {
                ...
                {
                    if (txtgerencia.Text.ToString().ToUpper() == valor.Cells["descripcion"].Value.ToString().ToUpper())
                    {
                ...
                //Insertando;
                CCargos.InsertarActualizarEmpresaEps(0, 1, txtgerencia.Text, frmLogin.CodigoUsuario);
            ...
            if (estado == 2)
            {
                ...
                foreach (DataGridViewRow valor in dtgconten.Rows)
                {
                    if (txtgerencia.Text.ToString().ToUpper() == valor.Cells["descripcion"].Value.ToString().ToUpper() && fila != dtgconten.CurrentCell.RowIndex)
                    {
                        Msg("LA Empresa ya Existe");
                        txtgerencia.Focus();
                        return;
                ...
                //modificando
                CCargos.InsertarActualizarEmpresaEps(int.Parse(txtcodigo.Text), 2, txtgerencia.Text, frmLogin.CodigoUsuario);
