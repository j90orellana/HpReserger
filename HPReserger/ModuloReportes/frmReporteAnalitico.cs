        ...
        private void btngenerar_Click(object sender, EventArgs e)
        {
                ...
                foreach (string cuentita in Cuentas)
                {
                    ...
                    if (Cuenta.Length == 2)
                    {
                        if (string.CompareOrdinal(Cuenta[1], Cuenta[0]) > 1)
                            Buscarcuenta += $" a.Cuenta_Contable  between '{Cuenta[0]}' and '{Cuenta[1]}9' or ";
            ...
            if (txtbusnrodoc.EstaLLeno())
            {
                ...
                foreach (string Doc in Documentos)
                {
                    ...
                    if (Docito.Length >= 1) { BuscarDocumento += $"concat(isnull(ax.Cod_Comprobante,''),'-',(isnull(ax.Num_Comprobante,'')))like  '%{Docito[0]}%' or "; }
                }
