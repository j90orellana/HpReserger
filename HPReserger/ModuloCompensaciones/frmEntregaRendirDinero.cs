...
{
    public partial class frmEntregaRendirDinero : FormGradient
    {
        public frmEntregaRendirDinero()
        {
            InitializeComponent();
        }
        ...
        string BanCuenta;
        int idTipocuenta;
        private void frmEntregaRendirDinero_Load(object sender, EventArgs e)
        {
            Cargado = false;
        ...
        private void BuscarEmpleadoCompensaciones()
        {
            ...
            {
                cboempleado.DataSource = CapaLogica.EntregasRendir_Detalle((int)cboempresa.SelectedValue);
        ...
        private void cboempleado_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                DataTable Table = CapaLogica.EntregasRendir_Detalle(cboempleado.SelectedValue.ToString(), (int)cboempresa.SelectedValue);
        ...
        private void Dtgconten_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.RowIndex >= 0)
            {
                ...
                {
                    frmReembolsoGastosDetalleFact frmdetallegastosfac = new frmReembolsoGastosDetalleFact((int)cboempresa.SelectedValue, (int)Dtgconten[xpkid.Name, e.RowIndex].Value, true);
        ...
        private void cboempleado_Click(object sender, EventArgs e)
        {
            if (cboempresa.SelectedValue != null)
            {
                DataTable Tablita = CapaLogica.EntregasRendir_Detalle((int)cboempresa.SelectedValue);
        ...
        private void btnaceptar_Click(object sender, EventArgs e)
        {
            ...
            if (msgp("¿Seguro Desea Hacer el Abono?") == DialogResult.Yes)
            {
                ...
                CapaLogica.InsertarAsientoFacturaDetalle(10, PosFila, numasiento, FechaContable, BanCuenta, proyecto, int.Parse(UserCompensa[0]), UserCompensa[1]
                  , cboempleado.Text.Substring(cboempleado.Text.IndexOf('-') + 2).ToUpper(), 0, "0", $"ER{FechaPago.ToString(Configuraciones.ddMMyy)}"  //$"{FechaPago.ToString("dd/MM/yyyy")} {Configuraciones.MayusculaCadaPalabra(cboempleado.Text.Substring(cboempleado.Text.IndexOf('-') + 2))}"
                ...
                foreach (DataGridViewRow item in Dtgconten.Rows)
                {
                    ...
                    if ((int)item.Cells[xok.Name].Value == 1)
                    {
                        CapaLogica.InsertarAsientoFacturaCabecera(1, ++PosFila, numasiento, FechaContable, CuentaContable, moneda == 1 ? MontoMN : MontoME, 0, TCPago, proyecto, 0, Cuo, moneda, Glosa, FechaPago, -25);
                        ...
                        //Pago del Reembolso
                        CapaLogica.EntregasRendir_Detalle(pkId, IdEmpresa, Cuo, nroKuenta, NroPago, NroPago, TipoPago, FechaPago, IdLogin);
                    }
                }
                //Cuadre Asiento
