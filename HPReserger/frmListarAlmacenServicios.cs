...
{
    public partial class frmListarAlmacenServicios : FormGradient
    {
        public frmListarAlmacenServicios()
        {
            InitializeComponent();
        }
        ...
        private void CargraCombo()
        {
            ...
            DataTable ListaOCProveedor = clFIG.ListarOCProveedorAprobadas(txtRUC.Text, 1, frmLogin.CodigoUsuario);
        ...
        private void cboOC_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (cboOC.SelectedValue != null)
            {
                ...
                {
                    double suma = 0;
                    for (int i = 0; i < gridDetalle1.RowCount; i++)
                    {
                        suma += Convert.ToDouble(gridDetalle1["ARTFIC", i].Value.ToString());
                    }
        ...
        private void Actualiza(int OC)
        {
            if (cboOC.SelectedValue != null)
            {
                gridDetalle1.DataSource = clFIG.ListarFIClistar(OC, 1);
        ...
        private void Actualiza2(int OC, int FIC)
        {
            if (OC != 0 && FIC != 0)
            {
                gridDetalle2.DataSource = clFIG.ListarFIC2listar(OC, FIC, 1);
            }
            else
            {
               // gridDetalle2.DataSource = null;
        ...
        private void gridDetalle1_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {

        }

        private void gridDetalle1_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
